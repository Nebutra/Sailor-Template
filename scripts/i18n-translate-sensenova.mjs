...
/**
 * Leaves whose correct translation IS the English string ("Wallet", "RAG",
 * "Audio") are otherwise re-sent on every run forever. Recording them here —
 * keyed on the English fingerprint — lets the queue actually reach zero, while
 * an edit to the English invalidates the record automatically.
 */
const CONFIRMED_PATH = join(REPO_ROOT, "i18n-confirmed-identical.json");

function loadConfirmed() {
  try {
    const raw = JSON.parse(readFileSync(CONFIRMED_PATH, "utf8"));
    return raw && typeof raw === "object" && raw.confirmed ? raw.confirmed : {};
  } catch {
    return {};
  }
}

function confirmedMapFor(store, catalogId, locale) {
  const entries = store?.[catalogId]?.[locale];
  return new Map(entries ? Object.entries(entries) : []);
}

function saveConfirmed(store) {
  const sortDeep = (o) =>
    Object.fromEntries(
      Object.entries(o)
  ...
  const body = {
    version: 1,
    note: "Leaves whose correct translation equals the English source. Values are md5 of the English string, so editing the English re-queues the leaf. Generated by scripts/i18n-translate-sensenova.mjs; safe to delete (costs one re-translation).",
...
async function translateLocale(catalog, targetLocale, sourceMap, { force, dryRun, confirmed }) {
  ...
  const confirmedForLocale = confirmedMapFor(confirmed, catalog.id, targetLocale);
  const work = collectWork(sourceMap, targetMap, { force, confirmedIdentical: confirmedForLocale });
    ...
    batches.map((batch, idx) =>
      limit(async () => {
        ...
        for (const [k, v] of accepted) {
          updates.set(k, v);
          // The model returned the source unchanged — that IS the translation
          // here. Record it so the leaf stops coming back every run.
          const src = sourceMap.get(k);
          if (typeof src === "string" && v === src) {
            ((confirmed[catalog.id] ??= {})[targetLocale] ??= {})[k] = sourceFingerprint(src);
          }
        }
...
async function main() {
  ...
  if (!args.dryRun) {
    try {
      saveConfirmed(args.confirmed);
    } catch {
      // non-fatal: a missing confirmation file only costs one re-translation
    }
  }
