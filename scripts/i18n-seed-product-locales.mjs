...
// Emitted element-per-line with a trailing comma rather than via
// JSON.stringify: JSON has no trailing commas, so the array's last entry came
// out `"zh-Hant"` where Biome's formatter wants `"zh-Hant",`. Every run of this
// script therefore produced a file that failed `pnpm lint`, and the first
// regeneration to reach main (#598) turned the whole repo's format check red.
...
${files.map((l) => `  ${JSON.stringify(l)},`).join("\n")}
] as const;
