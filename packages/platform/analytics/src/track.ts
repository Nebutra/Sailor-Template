 ...
 */
export function createProductAnalyticsClient(opts: AnalyticsClientOptions): ProductAnalyticsClient {
  ...
  return {
    async track(event, properties) {
      ...
      const distinctId =
        typeof data.userId === "string" && data.userId.length > 0 ? data.userId : "anonymous";
      const timestamp =
