...
describe("Event contracts", () => {
  ...
  it("LicenseWizardEvent rejects unknown tier", () => {
    expect(() => LicenseWizardEvent.parse({ step: "submitted", tier: "FREE" })).toThrow();
  });

  it("LicenseCliEvent requires cli_version", () => {
    expect(() => LicenseCliEvent.parse({ action: "activate_attempted" })).toThrow();
    expect(() =>
  ...
  it("SleptonsEvent restricts action to known values", () => {
    expect(() => SleptonsEvent.parse({ action: "profile_viewed" })).not.toThrow();
    expect(() => SleptonsEvent.parse({ action: "delete_account" })).toThrow();
  });

  it("DocsSearchEvent requires query and result_count", () => {
    expect(() => DocsSearchEvent.parse({ query: "rbac", result_count: 4 })).not.toThrow();
