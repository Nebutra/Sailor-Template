...
| Source files | `packages/brand/src/{positioning,metadata,motion}.ts`, `packages/brand/src/guidelines/{color,logo}.ts` |
| VI manual | `packages/brand/assets/vi/` (PDF + extracted markdown) |
...
Voice is **technical, founder-empathic, anti-fluff**. Every claim in marketing copy must be backed by an actual package or service in the monorepo (see `packages/brand/src/positioning.ts` `pillars` array).
...
- Logo assets, clear-space rules, and minimum sizes are in `packages/brand/assets/vi/`.
...
These are **TS source primitives**. The runtime equivalent lives in `@nebutra/tokens` — see [`packages/tokens/DESIGN.md`](../tokens/DESIGN.md).
...
> Note: `packages/brand/src/guidelines/color.ts` exports a 50–950 scale per anchor. The accent scale uses slightly different green-bias values than the runtime `@nebutra/tokens` cyan scale; the **runtime tokens** at `packages/tokens/styles.css` are the source of truth for what ships in apps. The brand `guidelines/color.ts` is the **design data** for VI / print / Figma.
...
### 2.3 Motion language (`packages/brand/src/motion.ts`)
...
Spring presets: `default | bouncy | heavy | gentle` (`packages/brand/src/motion.ts`).
...
- Logo SVG components are in `packages/brand/src/components/` but lack Storybook stories — recommend adding to the Foundation/Logo group.
