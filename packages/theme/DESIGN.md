...
| Source file | `packages/theme/themes.css` |
...
> Exact oklch tuples for every theme are in `packages/theme/themes.css`. Designer-readable per-theme tables are deferred to Storybook (Foundation/Themes) — adding them here would duplicate the source of truth.
