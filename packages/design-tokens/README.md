...
> **Coexistence, not replacement.** This package runs *alongside* `packages/tokens/styles.css`, `packages/theme/themes.css`, `packages/ui/src/tokens/`, and `packages/brand/src/`. Apps continue to consume the legacy SSOT until parity is at 100% and migrations land.
...
This keeps the generated CSS interchangeable with the manually-maintained `packages/tokens/styles.css` for ongoing parity verification.
...
`build/css/styles.generated.css` is the concatenation of `light.css` + `dark.css` and is the file that `verify-parity.ts` diffs against `packages/tokens/styles.css`.
...
## Why not edit `packages/tokens/styles.css` directly?
