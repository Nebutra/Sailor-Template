...
This package **consumes** tokens from `@nebutra/tokens` exclusively. It does not declare new tokens. Internal Lobe UI components are bridged via `NebutraThemeProvider` (`packages/ui/src/theme/`) which maps Nebutra brand tokens into Lobe UI's expected shape.
