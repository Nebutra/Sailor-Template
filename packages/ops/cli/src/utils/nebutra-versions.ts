 ...
 * correct caret range.
 *
 * Keep in sync with `packages/ops/create-sailor/src/utils/nebutra-versions.ts`.
 *
 ...
 *
 * Returns `null` when the package is not in the published set — callers
 * should decide whether to fall back to `workspace:*` (monorepo-internal)
 * or skip the dependency entirely (user-facing scaffold).
 */
export function getNebutraPackageVersion(packageName: string): string | null {
