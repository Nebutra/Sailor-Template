...
export function buildStartupRunPrompt(
  ...
  return JSON.stringify(
    {
      instruction:
        'Generate the requested Startup Agent OS artifact update. The workspace is a TanStack Start app PRE-WIRED with the Nebutra design system — it already depends on @nebutra/ui (components), @nebutra/tokens (src/styles/app.css imports "@nebutra/tokens/styles.css"), and @nebutra/icons. File-based routing lives under src/routes/ — src/routes/__root.tsx is the HTML shell (no index.html or src/main.tsx exists) and wraps the app in a next-themes ThemeProvider, src/routes/index.tsx is the home route, and global styles live in src/styles/app.css. To change the founder landing surface, patch src/routes/index.tsx; to change document head or shell, patch src/routes/__root.tsx. You MUST keep building UI with @nebutra/ui components (e.g. Button, Card, Badge from "@nebutra/ui/primitives") + @nebutra/icons glyphs and the brand CSS variables from @nebutra/tokens (var(--brand-gradient), var(--neutral-12), etc.); NEVER revert the founder surface to bare/unstyled HTML or hardcoded hex colors. Return strict JSON only with keys: summary, artifactUpdates, filePatches. filePatches may only update existing workspace paths shown in workspaceFiles (use those exact paths) and must include full replacement content. Never claim a deploy, send, payment, or production mutation happened.',
  ...
  const { result, provider } = await runWithFallback(
    async (model) =>
      generateAIText({
        model,
        system:
          'You are Nebutra Startup Agent OS. The editable workspace is a TanStack Start app PRE-WIRED with the Nebutra design system (@nebutra/ui + @nebutra/tokens + @nebutra/icons): file-based routing under src/routes/, src/routes/__root.tsx is the HTML shell (no index.html or src/main.tsx) wrapping a next-themes ThemeProvider, src/routes/index.tsx is the home route, and styles live in src/styles/app.css (which imports "@nebutra/tokens/styles.css"). Produce founder-grade artifacts from CompanyContext and target those real paths in filePatches. You MUST keep building UI with @nebutra/ui components (Button, Card, Badge from "@nebutra/ui/primitives") + @nebutra/icons glyphs and the @nebutra/tokens brand CSS variables; never revert the founder surface to bare HTML or hardcoded hex. Return strict JSON only.',
