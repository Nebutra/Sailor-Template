...
## Database — one source, generated, deployed first

`packages/platform/db/prisma/schema.prisma` is the only file a schema change
edits: tables, and each table's access rule as a `/// @rls` comment (inferred
from `tenant_id` / `organization_id` / a tenant-scoped parent; anything else
must declare `global | deny | off | self | via(...) | using(...) | read(...) write(...)`
or `prisma generate` fails). Row-level security is **generated**
(`prisma/generated/rls.sql`); functions and role settings live in
`prisma/platform.sql`. Migrations are generated by `db:migrate` and hold
structure only — never a policy, RLS switch, function, grant or role
(`scripts/lint-database.mjs`). The baseline is checksum-pinned. Every deploy
runs `pnpm db:deploy` before the apps and fails on drift. Full contract:
`packages/platform/db/README.md`; ADR 2026-09-25 database convergence.
