...
export const ElevationScale: StoryObj = {
  ...
  render: () => (
    <div className="grid grid-cols-3 gap-6 bg-muted p-8 md:grid-cols-6">
      {(["xs", "sm", "md", "lg", "xl", "2xl"] as const).map((size) => (
        <div key={size} className="flex flex-col items-center gap-3">
          <div
            className="h-20 w-full rounded-lg bg-background"
            style={{ boxShadow: `var(--elevation-${size})` }}
          />
          <span className="font-mono text-xs text-muted-foreground">--elevation-{size}</span>
        </div>
      ))}
    </div>
  ),
...
export const BrandShadows: StoryObj = {
  name: "Brand Shadows",
  render: () => (
    <div className="grid grid-cols-2 gap-8 bg-foreground p-8">
      {[
        ...
        <div key={name} className="flex flex-col items-center gap-3">
          <div
            className="h-24 w-full rounded-lg bg-primary"
