...
/**
 * Product chrome brand mark.
 * Primary: /public/brand (synced via `pnpm brand:sync` / Vercel buildCommand).
 * Fallback: inline SVG if the public asset 404s (gitignored public/brand).
 */
...
function FallbackMark({ wordmark }: { wordmark?: boolean }) {
  const gradId = wordmark ? "nbBrandHorizGrad" : "nbBrandMarkGrad";
  return (
    <svg
