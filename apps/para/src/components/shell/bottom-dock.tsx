...
export function BottomDock() {
  ...
  return (
    <div className="pointer-events-none absolute inset-x-0 bottom-4 flex justify-center">
      <div className="para-rise pointer-events-auto flex h-[var(--para-dock-h)] w-para-dock items-center gap-1 rounded-full border border-border bg-popover pr-1.5 pl-1.5 shadow-ambient-md">
        <button
          type="button"
          onClick={() => toggleDrawer("library")}
          aria-pressed={activeDrawer === "library"}
          className="h-[var(--para-h-control)] rounded-full px-3.5 text-body text-foreground hover:bg-accent aria-pressed:bg-accent"
        >
          Assets
        </button>
        <button
          ...
          className="flex h-[var(--para-h-control)] flex-1 items-center rounded-full px-4 text-left text-muted-foreground text-body hover:bg-accent hover:text-foreground"
        >
          Ask PARA…
        </button>
        <button
