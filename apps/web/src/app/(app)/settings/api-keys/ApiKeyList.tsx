  ...
  return (
    <form action={action}>
      ...
      <button
        type="submit"
        disabled={isPending}
        className="text-xs text-destructive-strong hover:text-destructive-strong/80 disabled:opacity-50"
      >
    ...
    <ul className="divide-y divide-border">
      {keys.map((k) => (
        <li key={k.id} className="py-3 flex items-center justify-between">
          ...
          {k.revokedAt ? (
            <span className="rounded-full border border-destructive/30 bg-destructive/10 px-2 py-0.5 text-xs font-medium text-destructive-strong">
              Revoked
            </span>
          ) : (
