/**
 * Playwright config for the **golden-path** suite under `e2e/golden/`.
 *
 * Sister configs in this directory: `playwright.config.ts` (smoke, with
 * managed webServer) and `playwright.sleptons.config.ts` (sleptons-only).
 *
 * This config is intentionally lighter — it does NOT spawn a dev server.
 * Run it with the web app + landing page already up (or set `APP_BASE_URL`
