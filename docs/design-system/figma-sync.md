...
> **Status:** Phase 2 — provider abstraction in place, validation green, Figma **push still disabled by default**. The Figma transport is now one of three providers wired through [`@nebutra/design-sync`](../../packages/design-sync/README.md): `figma` (Tokens Studio plugin), `penpot` (China-friendly), `git-only` (zero-config). See [`packages/design-sync/DESIGN.md`](../../packages/design-sync/DESIGN.md) for the architecture.
...
**Source of truth:** `packages/design-tokens/tokens/*.json` (W3C DTCG `$value` / `$type`).
**Runtime consumer:** `packages/tokens/styles.css` — generated from DTCG, parity-checked in CI.
...
5. CI (`design-sync.yml`) runs:
   ...
   - parity check against `packages/tokens/styles.css`
...
1. Edit `packages/design-tokens/tokens/<set>.json` directly.
...
| Trigger                                        | Behaviour                                      |
| ---------------------------------------------- | ---------------------------------------------- |
| Push / PR touching `packages/design-tokens/**` | Validate + build + parity + PR comment         |
| Push / PR touching `.tokens-studio/**`         | Validate metadata only                         |
| `packages/design-tokens/tokens/` is empty      | `validate` job exits early — no failures, no diff comment |
...
5. Wire up the live push inside `packages/design-sync/src/providers/figma.ts` (the provider currently throws "live push not yet implemented" so an accidental run cannot wipe a Figma file). The implementation should call:
...
`packages/tokens/styles.css` no longer matches the generated CSS from `packages/design-tokens`. Either:

- regenerate the runtime CSS from DTCG (preferred), or
- backport the manual edit into the appropriate `tokens/**.json` file.

The verifier script lives at `packages/design-tokens/scripts/verify-parity.ts`.

### "Tokens Studio cannot pull from git"

- Confirm the plugin's **Sync provider = GitHub** with branch `main`, file path `packages/design-tokens/tokens`, and multi-file mode enabled.
...
| Path                                       | Purpose                                          |
| ------------------------------------------ | ------------------------------------------------ |
| `packages/design-sync/`                    | Provider-agnostic sync package (figma/penpot/git-only/memory) |
| `packages/design-sync/src/cli/index.ts`    | `design-sync` CLI (`detect`, `healthcheck`, `pull`, `push`)   |
...
| `packages/design-tokens/tokens/`           | DTCG source files (created by sibling agent)     |
| `packages/design-tokens/build/`            | Style Dictionary outputs (CSS / TS / JSON)       |
| `packages/tokens/styles.css`               | Runtime CSS variables consumed by all apps      |
