...
#
# 使用方法:
#   chmod +x infra/ops/scripts/deploy-ecs-lite.sh
#   sudo bash infra/ops/scripts/deploy-ecs-lite.sh
#
# 部署架构:
...
# 复制 ECS Lite 专用配置
cp "$PROJECT_DIR/infra/runtime/nginx/nginx-ecs-lite.conf" /etc/nginx/nginx.conf
