#!/usr/bin/env bash
# Point grey-cloud origin.nebutra.com at the Fly Hono origin.
#
# This hostname must stay DNS-only. The edge Worker fetches it (or the Fly
# app URL). Orange-clouding origin.nebutra.com is safe from a Worker loop
# today (the route is only api.nebutra.com), but it reintroduces the 525
# window and hides origin TLS from us. Keep it grey.
#
# Usage: FLY_APP=nebutra-gateway ./infra/ops/scripts/point-origin-dns-fly.sh
set -euo pipefail

TOKEN="${CLOUDFLARE_API_TOKEN:?CLOUDFLARE_API_TOKEN required}"
ZONE_NAME="${CF_ZONE_NAME:-nebutra.com}"
HOST="${HOST:-origin}"
FLY_APP="${FLY_APP:?FLY_APP is the Fly app name, e.g. nebutra-gateway}"
...
BODY=$(python3 -c "import json; print(json.dumps({'type':'CNAME','name':'${HOST}','content':'${TARGET}','proxied':False,'ttl':1,'comment':'fly ${FLY_APP} grey-cloud origin'}))")
EXIST=$(curl -sS -H "Authorization: Bearer $TOKEN" \
...
echo "$RESP" | python3 -c 'import json,sys; d=json.load(sys.stdin); assert d.get("success"), d.get("errors") or d; print("cname", d["result"]["name"], d["result"]["content"], "proxied", d["result"]["proxied"])'
