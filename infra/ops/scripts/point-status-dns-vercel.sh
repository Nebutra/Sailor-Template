#!/usr/bin/env bash
# Point status.nebutra.com → cname.vercel-dns.com (proxied orange-cloud).
#
# Status is a landing host alias (apps/landing proxy STATUS_HOST rewrites
# / → /status). It must NOT point at ECS: without a status server_name the
# default 443 block answers `301 → https://nebutra.com$request_uri`.
#
# After DNS:
#   vercel domains add status.nebutra.com --project nebutra-landing --scope nebutra
#
# Body rules (match point-www / point-carina):
...
HOST="status.${ZONE_NAME}"
CONTENT="${STATUS_DNS_TARGET:-cname.vercel-dns.com}"
...
echo "=== existing status CNAME records ==="
EXIST=$(curl -sS -H "Authorization: Bearer ${TOKEN}" \
  "https://api.cloudflare.com/client/v4/zones/${ZONE_ID}/dns_records?name=${HOST}&type=CNAME")
echo "$EXIST" | python3 -m json.tool | head -40

BODY=$(python3 -c "import json; print(json.dumps({'type':'CNAME','name':'status','content':'${CONTENT}','proxied':True,'ttl':1}))")
...
if not d.get("success"):
    ...
    if 10000 in codes:
        ...
        print("::error::Until then set status CNAME cname.vercel-dns.com (proxied) in CF dashboard.")
    raise SystemExit(1)
r=d.get("result") or {}
...
curl -sSI --max-time 20 "https://${HOST}/status.json" | head -15 || true
echo "done"
