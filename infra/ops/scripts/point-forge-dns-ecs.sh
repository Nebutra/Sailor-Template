#!/usr/bin/env bash
# Point forge.nebutra.com → ECS origin (A, proxied).
set -euo pipefail
...
BODY=$(python3 -c "import json; print(json.dumps({'type':'A','name':'forge','content':'${ORIGIN_IP}','proxied':True,'ttl':1,'comment':'nebutra-forge product edge'}))")
...
# Drop conflicting CNAME if present (Vercel leftovers)
