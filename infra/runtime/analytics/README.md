...
```bash
# From repo root
cp infra/runtime/analytics/env.example infra/runtime/analytics/.env     # then edit secrets
...
Runs `infra/runtime/analytics/smoke-test.ts` via Vitest and checks:
...
- **Never** commit a real `.env` file. `infra/runtime/analytics/.env` is in `.gitignore`.
