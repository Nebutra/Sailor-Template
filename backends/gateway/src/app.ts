...
export interface CreateGatewayAppOptions {
  /**
   * Register queue-backed background workers (completion worker, para-agent
   * worker) while building the app.
   *
   * The standalone server entry (src/node.ts) needs this — it is a
   * long-lived process whose job includes draining those queues. An app
   * embedded inside a Next.js request handler
   * (apps/web `app/api/[[...route]]/route.ts`, GATEWAY_MODE=embedded) must
   * NOT start workers as a side effect of serving an HTTP request: Next's
   * process lifecycle is request-driven, and doing so risks registering the
   * same in-process consumer repeatedly across route-handler
   * invocations/reloads.
   *
   * In embedded mode, queued/background work still flows — through the
   * QStash webhook delivery route (routes/queue/delivery.ts), mounted
   * either way, which processes one job per HTTP delivery rather than
   * through an in-process consumer loop.
   *
   * Defaults to true so the standalone entry's behavior is unchanged.
   */
...
export interface GatewayApp {
  app: OpenAPIHono;
  areGatewayDepsInitialized: () => boolean;
}

/**
 * Build the Hono gateway app: register all middleware and routes, and
 * (unless `startWorkers: false`) the background queue workers.
 *
 * The process-level singletons above this function (OTel, Sentry,
 * alerting-channel registration, the @nebutra/db-backed wiring) still run
 * once at module import time, exactly as before this refactor — they are
 * idempotent configuration, not per-call side effects, and both the
 * standalone entry and the Next mount want them applied exactly once per
 * process. Everything that is genuinely per-instance (the Hono app, its
 * routes, and optionally its workers) is built inside this factory instead
 * of at module scope, so a caller can build the app without also starting
 * background workers.
 */
export async function createGatewayApp(options: CreateGatewayAppOptions = {}): Promise<GatewayApp> {
  ...
  app.use("*", (c, next) => {
    ...
    return isPebbleDiagnosticUpload
      ? pebbleDiagnosticBodyLimit(c, next)
  ...
  return { app, areGatewayDepsInitialized: () => gatewayDepsInitialized };
}
